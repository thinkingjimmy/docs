...
また、[Comflowy](https://app.comflowy.com/app/s-flux-dev-workflow-db9bd4) で直接ワークフローを使用するか、このワークフローテンプレートをダウンロードしてインポートします。（また、Comflowy テンプレートの使用方法がわからない場合は、この [チュートリアル](https://intercom.help/comflowy/en/articles/10229083-how-do-i-get-the-workflow-template) を読んでください）。
...
私は ComfyUI の公式バージョンが少し複雑であると感じているため、より簡単なバージョンをお勧めします。このバージョンでは、Stable Diffusion で一般的に使用される KSampler ノードのみを使用し、ModelSamplingFlux ノードを削除します。そのため、SD3LatentImage で幅と高さを設定するだけで済みます。一般的に、このバージョンはより簡単で理解しやすいです。
...
以下のスクリーンショットを参照してノードを接続するか、[Comflowy](https://app.comflowy.com/app/s-flux-dev-fp8-workflow-db9bd4) からこのワークフローをダウンロードしてインポートします。

このバージョンは、前にお勧めしたバージョンよりもさらに簡単なバージョンです。Diffusion Model、DualCLIPLoader、および VAE ノードを Checkpoint Loader ノードに置き換えます。
...
プラグインをインストールした後、以下のスクリーンショットを参照してノードを接続するか、[Comflowy](https://app.comflowy.com/app/s-flux-dev-gguf-workflow-db9bd4) からこのワークフローをダウンロードしてインポートします。
